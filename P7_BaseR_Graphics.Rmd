...
Welcome to Part 7 of the Introduction to R Workshop.
If you have worked your way through the previous
modules, by now you are missing one of the best and most exciting
aspects of the R language.
We will dedicate this tutorial to exploring this powerful feature
for exploring, communicating and explaining data and mathematics 
in general.

Base R graphics are powerful, fast, and require no additional
packages [@idiotsGuideGraphicsBaseR]. 
While packages like ggplot2 offer advanced features
[@r-graphicsChang], base R graphics provide an excellent starting
point and remain widely used for quick exploratory analysis and
publication-quality figures.

By the end of this tutorial, you will understand the basic graphics 
capabilities in base R. You will learn how to create line plots of 
mathematical functions, histograms of simulated data, and diagnostic 
plots to assess normality of distributions. These fundamental plotting 
skills form the foundation for data visualization in R and will 
help you explore and communicate your data effectively.
...
From module 1 in this workshop series you may remember how 
to summon documentation for the function `plot`. 
It turns out `plot` is a generic function that dispatches
calls to specific functions depending on the type of the
objects passed as main argument, `x`.

When the funcion gets called with two R numeric vectors,
`plot.default` is used.
Observe that the output of `help(plot)` list the "generic"
`plot` from the package base and the default scatterplot
function from the package graphics.

```{r help-plot, exercise=TRUE, exercise.lines=2}
help(plot)
#help(plot.default) # this is the function for scatter plots
```

The following is an example taken from the documentation
of the `plot` function used for scatter plots. 
You can get it straight from the R system by uncommenting the
second line above and scrolling down to the examples
section.

The code iterates over the character vector on line 4 
representing all the possible types of x-y plots possible
in this library.

```{r plot-types, exercise=TRUE}
x <- 0:15
y <- sin(pi/5 * x)
...
dev.off() # restore the layout to defaults
```

The built-in function `par` deserves a full explanation on
its own merit. 
Basically it defines the layout and other properties of the
graphic device.
...
Here is a quick inventory of the builtin functions we used in 
this section of the Introduction to R workshop. 
All these functions are from the rich Base R package. 
...
## 📌 Tutorials &  online-resources
  ...
  6. https://rstudio-pubs-static.s3.amazonaws.com/7953_4e3efd5b9415444ca065b1167862c349.html (bare bones)
  ...
  5. https://education.rstudio.com/learn/ (A road map in R through Posit)
  ...
  9. https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html (the R pipe operator)
